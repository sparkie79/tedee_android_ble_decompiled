...
import static tedee.mobile.sdk.ble.bluetooth.secure.SecureConnectionConstants.LEN_AUTH_TAG;
...
class MessageCipher {
    ...
    protected static int getLength(byte[] message, int offset) {
        return ((message[offset] & 0xff) << 8) | (message[offset + 1] & 0xff);
    }

    protected static void setLength(byte[] message, int offset, int len) {
     ...
     */
    protected MessageCipher(SecretKey sharedSecret, byte[] label, byte[] data, int mode)
    ...
    protected MessageCipher(SecretKey sharedSecret, byte[] label, byte[] data, int mode, CipherData cipherData)
            throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException {
        // calculate key and iv
        Mac mac = Mac.getInstance("HmacSHA256");
        mac.init(sharedSecret);
     ...
     */
    protected byte[] transform(byte[] message, int len)
        ...
        GCMParameterSpec parameterSpec = new GCMParameterSpec(SecureConnectionConstants.LEN_AUTH_TAG * 8, iv, 0, 12);
